...
void main(void)
{
    ...
    *IX += 1;      /* Z[2] se vuelve a modificar, ahora vale 12. */
    ...
    X = *(IX + 1);  /* IX se modifica. Observa la forma de mover el apuntador.
    Ahora IX apunta al cuarto elemento de Z (Z[3]). */
    Y = *IX;       /* Y se modifica, ahora vale Z[3] (8). */
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);

    IX = IX + 4;     /* IX se modifica. Observa que el apuntador se mueve 4 posiciones y cae en una direccion afuera del arreglo. Esto ocasionara un
    ...
    IX = IX + 1;   /* IX se mueve una posicion y cae en una celda incorreta. */
