...
void interpreta(char *);        /* Prototipos de funcion. */
...
/* Esta funcion se utiliza para decodificar la cadena caracteres. */
{
    int i = 0, j, k;
    while (cadena[i] != '\0')
    {
        if (isalpha (cadena[i]))   /* Se utiliza isalpha para observar si el caracter es una letra. */
        {
            ...
            en caracter- que nos interesa, menos 48 que corresponde al ascii del
            digito 0. */
            for (j = 0; j < k; j++)
                putchar(cadena[i]);
        }
