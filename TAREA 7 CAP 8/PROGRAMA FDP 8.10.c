#include <stdio.h>
#include <string.h>

/* Vendedores.
   El programa maneja información sobre las ventas que realizan los vendedores
   de artículos domésticos de una empresa de la Ciudad de México. */

/* Estructura banco */
typedef struct
{
    char noba[10];  /* Nombre del banco */
    char nucu[10];  /* Número de cuenta */
} banco;

/* Unión fpago */
typedef union
{
    banco che;      /* Cheque: pago por cuenta de cheques */
    banco nomi;     /* Nómina: depósito por nómina */
    char venta;     /* Ventanilla: pago directo en ventanilla */
} fpago;

/* Estructura domicilio */
typedef struct
{
    char cnu[20];   /* Calle y número */
    char col[20];   /* Colonia */
    char cp[5];     /* Código Postal */
    char ciu[15];   /* Ciudad */
} domicilio;

/* Estructura vendedor */
typedef struct
{
    int num;        /* Número del vendedor */
    char nom[20];   /* Nombre del vendedor */
    float ven[12];  /* Ventas del año (12 meses) */
    domicilio domi; /* Domicilio del vendedor */
    float sal;      /* Salario mensual */
    fpago pago;     /* Forma de pago (unión) */
    int cla;        /* Clave forma de pago (1,2,3) */
} vendedor;

/* Prototipos de funciones */
void Lectura(vendedor *, int);  /* Lectura de los datos del arreglo */
void F1(vendedor *, int);       /* Ventas totales */
void F2(vendedor *, int);       /* Incremento salarial */
void F3(vendedor *, int);       /* Vendedores con pocas ventas */
void F4(vendedor *, int);       /* Vendedores pagados por banco */

void main(void)
{
    vendedor VENDEDORES[100];  /* Arreglo de vendedores */
    int TAM;

    /* Lectura del tamaño del arreglo */
    do
    {
        printf("Ingrese el numero de vendedores: ");
        scanf("%d", &TAM);
    }
    while (TAM > 100 || TAM < 1); /* Validación */

    /* Lectura general */
    Lectura(VENDEDORES, TAM);

    /* Procesos */
    F1(VENDEDORES, TAM);
    F2(VENDEDORES, TAM);
    F3(VENDEDORES, TAM);
    F4(VENDEDORES, TAM);

    printf("\n\tFIN DEL PROGRAMA");
}

/* -------------------------------------------------------------------------- */
/* LECTURA DEL ARREGLO COMPLETO                                                */
/* -------------------------------------------------------------------------- */
void Lectura(vendedor A[], int T)
{
    int I, J;

    for (I = 0; I < T; I++)
    {
        printf("\n\tIngrese datos del vendedor %d", I + 1);

        printf("\nNúmero de vendedor: ");
        scanf("%d", &A[I].num);

        printf("Nombre del vendedor: ");
        fflush(stdin);
        gets(A[I].nom);

        /* Lectura de las ventas del año */
        printf("Ventas del año:\n");
        for (J = 0; J < 12; J++)
        {
            printf("\tMes %d: ", J + 1);
            scanf("%f", &A[I].ven[J]);
        }

        /* Lectura del domicilio */
        printf("Domicilio del vendedor:\n");
        printf("\tCalle y número: ");
        fflush(stdin);
        gets(A[I].domi.cnu);

        printf("\tColonia: ");
        fflush(stdin);
        gets(A[I].domi.col);

        printf("\tCódigo Postal: ");
        fflush(stdin);
        gets(A[I].domi.cp);

        printf("\tCiudad: ");
        fflush(stdin);
        gets(A[I].domi.ciu);

        /* Salario */
        printf("Salario del vendedor: ");
        scanf("%f", &A[I].sal);

        /* Forma de pago */
        printf("Forma de Pago (Banco-1 Nómina-2 Ventanilla-3): ");
        scanf("%d", &A[I].cla);

        /* Campo tipo unión */
        switch (A[I].cla)
        {
            case 1: /* Cheque */
                printf("\tNombre del banco: ");
                fflush(stdin);
                gets(A[I].pago.che.noba);

                printf("\tNúmero de cuenta: ");
                fflush(stdin);
                gets(A[I].pago.che.nucu);
                break;

            case 2: /* Nómina */
                printf("\tNombre del banco: ");
                fflush(stdin);
                gets(A[I].pago.nomi.noba);

                printf("\tNúmero de cuenta: ");
                fflush(stdin);
                gets(A[I].pago.nomi.nucu);
                break;

            case 3: /* Ventanilla */
                A[I].pago.venta = 'S';
                break;
        }
    }
}

/* -------------------------------------------------------------------------- */
/* F1: CALCULA LAS VENTAS TOTALES ANUALES DE CADA VENDEDOR                    */
/* -------------------------------------------------------------------------- */
void F1(vendedor A[], int T)
{
    int I, J;
    float SUM;

    printf("\n\t\tVentas Totales de los Vendedores");

    for (I = 0; I < T; I++)
    {
        printf("\nVendedor: %d", A[I].num);

        SUM = 0;
        for (J = 0; J < 12; J++)
            SUM += A[I].ven[J];

        printf("\nVentas: %.2f\n", SUM);
    }
}

/* -------------------------------------------------------------------------- */
/* F2: INCREMENTA EL SALARIO A QUIENES VENDIERON MÁS DE 1,500,000             */
/* -------------------------------------------------------------------------- */
void F2(vendedor A[], int T)
{
    int I, J;
    float SUM;

    printf("\n\t\tIncremento a los Vendedores con Ventas > 1,500,000$");

    for (I = 0; I < T; I++)
    {
        SUM = 0;
        for (J = 0; J < 12; J++)
            SUM += A[I].ven[J];

        if (SUM > 1500000.00)
        {
            A[I].sal *= 1.05; /* Aumento del 5% */

            printf("\nNúmero de empleado: %d\nVentas: %.2f\nNuevo salario: %.2f",
                   A[I].num, SUM, A[I].sal);
        }
    }
}

/* -------------------------------------------------------------------------- */
/* F3: LISTA VENDEDORES QUE VENDIERON MENOS DE 300,000                        */
/* -------------------------------------------------------------------------- */
void F3(vendedor A[], int T)
{
    int I, J;
    float SUM;

    printf("\n\t\tVendedores con Ventas < 300,000");

    for (I = 0; I < T; I++)
    {
        SUM = 0;
        for (J = 0; J < 12; J++)
            SUM += A[I].ven[J];

        if (SUM < 300000.00)
            printf("\nNumero de empleado: %d\nNombre: %s\nVentas: %.2f",
                   A[I].num, A[I].nom, SUM);
    }
}

/* -------------------------------------------------------------------------- */
/* F4: IMPRIME LOS VENDEDORES QUE COBRAN POR CHEQUE (FORMA DE PAGO 1)        */
/* -------------------------------------------------------------------------- */
void F4(vendedor A[], int T)
{
    int I;

    printf("\n\t\tVendedores con Cuenta en el Banco");

    for (I = 0; I < T; I++)
        if (A[I].cla == 1)
            printf("\nNumero de vendedor: %d\nBanco: %s\nCuenta: %s",
                   A[I].num, A[I].pago.che.noba, A[I].pago.che.nucu);
}
