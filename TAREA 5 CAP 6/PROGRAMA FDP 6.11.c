...
/* REN es un arreglo unidimensional de tipo real que se utilizara para almacenar eel redimiento anual de los fondos de inversion. */
LecturaM(FON, 5, 12);
LecturaV(PRE, 5);
F1(FON, 5, 12, PRE, REN);
F2(FON, 5, 12);
...
int I;
printf("\nREDIMIENTOS ANUALES DE LOS FONDOS");
for(I = 0; I < F; I++)
{
...
printf("\nMejor fondo:  %d\tRedimiento: %6.2f", M + 1, ME);
printf("\nPeor fondo: %d\tRedimiento: %6.2f", P + 1, PE);
}
